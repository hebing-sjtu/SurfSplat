...
def rescale_and_crop(
    ...
    intrinsics: Float[Tensor, "*#batch 3 3"],
    shape: tuple[int, int],
    depths: None | Float[Tensor, "*#batch h_in w_in"],
) -> (
    tuple[
        Float[Tensor, "*#batch c h_out w_out"],  # updated images
        Float[Tensor, "*#batch 3 3"],  # updated intrinsics
...
def apply_crop_shim_to_views(views: AnyViews, shape: tuple[int, int]) -> AnyViews:    
    if "depth" in views and views["depth"] is not None:
        # print("views_depth_shape:",views["depth"].shape)
        images, intrinsics, depths = rescale_and_crop(
            views["image"], views["intrinsics"], shape, depths=views["depth"]
        )
        return {
            **views,
            "image": images,
