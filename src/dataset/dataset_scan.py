...
class DatasetScannet(IterableDataset):
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                scene = example["key"]
                print(f"Scene name: {scene}, run index: {run_idx}")
                try:
                ...
                # print(f"context_indices:", context_indices, context_indices[0].item())
                # Load the images.
                context_images = [
                ...
                if "depths" not in example:
                    print(f"Missing depths in {example['key']}")
                    continue
                # Load the depths
                context_depths = [
    ...
    def convert_depths(
        ...
        torch_depths = []
        for depth in depths:
            img = Image.open(BytesIO(depth.numpy().tobytes()))
            depth_arr = np.array(img).astype(np.float32) / 1000.0  # 假设单位是mm
            torch_depths.append(torch.tensor(depth_arr, dtype=torch.float32))
