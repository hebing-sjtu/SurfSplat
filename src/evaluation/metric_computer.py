...
from .metrics import compute_lpips, compute_psnr, compute_ssim, compute_depth_errors
...
class MetricComputer(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        try:
            for method in self.cfg.methods:
                depths = [
                    load_image(method.path / scene / f"depth/{index.item():0>6}.png")
                    for index in batch["target"]["index"][0]
                ]
                all_depths[method.key] = torch.stack(depths).to(self.device)
        except FileNotFoundError:
            print(f'No depth images for "{scene}", skipping depth metrics.')
            has_depth = False
        ...
        for key, images in all_images.items():
            ...
            if has_depth and key in all_depths:
                depth_gt = batch["target"]["depth"][0] if "depth" in batch["target"] else None
                if depth_gt is not None:
                    depth_pred = all_depths[key]
                    abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3 = compute_depth_errors(depth_gt, depth_pred)
                    all_metrics[f"rel_{key}"] = abs_rel.mean()
                    all_metrics[f"a1_{key}"] = a1.mean()
